...
export const createEvenements =  async ({titre, description, lieu, date_debut, date_fin}) => {
    try {
        await db.query('INSERT INTO evenements (titre, description, lieu, date_debut, date_fin) VALUES (?, ?, ?, ?, ?)', [titre, description, lieu, date_debut, date_fin]);
    }catch (error){
...
export const deleteEvenements = async (id) => {
    try {
        const [evenement] = await db.query('DELETE FROM evenements  WHERE id = ?',[id]);
