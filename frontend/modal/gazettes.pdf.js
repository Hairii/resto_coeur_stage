import { fetchGazettes, getPdfUrl } from "../api/gazettes.api.js";
    ...
    <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200">
      ...
      <div class="flex gap-3 items-center">
        <a id="modal-download" href="#" download
           class="bg-rose text-white text-xs font-bold uppercase px-4 py-2 rounded hover:opacity-85 transition-opacity">
          Télécharger
        </a>
...
const openPdfModal = (titre, pdfUrl) => {
...
const renderGazettes = (gazettes) => {
  ...
  gazettes.forEach((gazette) => {
    const pdfUrl = getPdfUrl(gazette.fichier_pdf);
    ...
    card.querySelector(".voir-btn").addEventListener("click", (e) => {
      openPdfModal(e.target.dataset.titre, e.target.dataset.url);
    });
