    ...
    gazettes.forEach((gazette) => {
      ...
      card.innerHTML = `
        ...
        <span class="text-xs text-gray-400">${new Date(gazette.created_at).toLocaleDateString("fr-FR")}</span>
         <a href="${API_URL}/uploads/gazettes/${gazette.fichier_pdf}" target="_blank" class="mt-auto bg-rose text-white text-xs font-bold uppercase px-4 py-2 rounded hover:opacity-85 transition-opacity text-center">
          Télécharger
        </a>
      `;
